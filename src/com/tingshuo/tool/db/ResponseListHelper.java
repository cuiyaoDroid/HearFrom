...
public class ResponseListHelper extends DBHelper {
	private final static String TABLE_NAME = "response_order";
	...
	public ResponseListHelper(Context context) {
		super(context);
	}
	...
	public long insert(ResponseListHolder content, SQLiteDatabase db) {
		ContentValues cv = new ContentValues();
		if (content.getId() != -1) {
			cv.put(ID, content.getId());
		}
		cv.put(USER_ID, content.getUser_id());
		cv.put(POST_ID, content.getPost_id());
	...
	private ResponseListHolder getDataCursor(Cursor cursor) {
		int id_column = cursor.getColumnIndex(ID);
		int user_id_column = cursor.getColumnIndex(USER_ID);
		int post_id_column = cursor.getColumnIndex(POST_ID);
		...
		int post_id = cursor.getInt(post_id_column);
	...
	public ArrayList<ResponseListHolder> selectData(int post_id,int from,int pagesize,int role_id,int sex) {
		SQLiteDatabase db = this.getReadableDatabase();
		Cursor cursor = db.query(TABLE_NAME, null, POST_ID+"=?"
				,new String[]{String.valueOf(post_id)}, null, null, ID + " asc limit "
						+ from + "," + pagesize);
		ArrayList<ResponseListHolder> holderlist = new ArrayList<ResponseListHolder>();
		for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
			ResponseListHolder holder=getDataCursor(cursor);
			holderlist.add(holder);
		}
		cursor.close();
		return holderlist;
	}
	public ArrayList<ResponseListHolder> selectSearchData(String num) {
		SQLiteDatabase db = this.getReadableDatabase();
	...
	public ArrayList<ResponseListHolder> selectData_commentid(int commentid) {
		SQLiteDatabase db = this.getReadableDatabase();
		Cursor cursor = db.query(TABLE_NAME, null, POST_ID
				+ "=?"
				,new String[]{String.valueOf(commentid)},null , null, ID + " desc limit 0,2");
		ArrayList<ResponseListHolder> holderlist = new ArrayList<ResponseListHolder>();
		for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
			ResponseListHolder holder=getDataCursor(cursor);
			holderlist.add(holder);
	...
	public ResponseListHolder selectData_Id(int v_id) {
		SQLiteDatabase db = this.getReadableDatabase();
