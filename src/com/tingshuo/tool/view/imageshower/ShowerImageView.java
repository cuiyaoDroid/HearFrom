...
public class ShowerImageView extends ImageView {
	...
	//
	// This matrix reImageDialogs the same when we go from the thumbnail image
	...
	protected void onDraw(Canvas canvas) {
		...
		if (width > ImageDialog.screenWidth) {
			// 如果图宽大于屏宽，就不用水平居中
			center(false, true);
		} else {
			center(true, true);
	...
	private void arithScaleRate() {
		float scaleWidth = ImageDialog.screenWidth / (float) imageWidth;
		float scaleHeight = ImageDialog.screenHeight / (float) imageHeight;
	...
	public void setImageBitmap(Bitmap bitmap) {
		...
		arithScaleRate();
		// 缩放到屏幕大小
		zoomTo(scaleRate, ImageDialog.screenWidth / 2f,
				ImageDialog.screenHeight / 2f);
		// 居中
		layoutToCenter();
		// imageView.zoomTo(scaleRate, ImageDialog.screenWidth / 2,
		// ImageDialog.screenHeight / 2
		// center(true, true);
	}
	...
	public void layoutToCenter() {
		...
		// 空白区域宽高
		float fill_width = ImageDialog.screenWidth - width;
		float fill_height = ImageDialog.screenHeight - height;
	...
	protected float getValue(Matrix matrix, int whichValue) {
		...
		mMinZoom = (ImageDialog.screenWidth / 2f) / imageWidth;
